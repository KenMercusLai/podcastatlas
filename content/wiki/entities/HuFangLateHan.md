---
title: "胡方 / Hu Fang (late Han)"
type: entity
tags: [person, late-han, official, frontier]
sources:
  - zizhi-tongjian-hanji-870-gudai-tizhinei-buganle-meiyou-tuilu-lpwbniu9ifpt6cp4ip9czqwsh5xd
last_updated: 2026-08-28
knowledge_schema: synthesis-v1
---

## Overview

胡方 / Hu Fang is a late-Han official who enters the wiki through 汉纪 870 as the Bingzhou inspector in [[HuangfuGuiLateHan|皇甫规]]'s attempted exit case. His importance is narrow: he recognizes that Huangfu Gui is trying to trigger impeachment so he can escape a frontier appointment, and he refuses to participate.

## Current Profile

Hu Fang's current profile is source-bounded to a single episode. He does not appear as a developed policy thinker or frontier commander; he appears as the official whose non-action blocks another official's manufactured exit. In the source's title-level reading, that makes him the institutional hinge between personal unwillingness and court demand for useful talent.

## Key Characteristics

- Bingzhou inspector in the 汉纪 870 appointment field.
- Recognizes Huangfu Gui's jurisdiction-crossing as a deliberate self-impeachment tactic.
- Refuses to validate the tactic by reporting Huangfu Gui to the court.
- Functions as the immediate official witness for office-duty exit constraint.
- Remains source-bounded; no wider biography is established by the current wiki.

## Evidence

- Office and setting: [[zizhi-tongjian-hanji-870-gudai-tizhinei-buganle-meiyou-tuilu-lpwbniu9ifpt6cp4ip9czqwsh5xd|汉纪 870]] identifies Hu Fang as Bingzhou inspector during Huangfu Gui's return to the Duliao general appointment.
- Recognition of tactic: [[zizhi-tongjian-hanji-870-gudai-tizhinei-buganle-meiyou-tuilu-lpwbniu9ifpt6cp4ip9czqwsh5xd|汉纪 870]] says Hu Fang sees that Huangfu Gui wants to leave office early by inviting impeachment.
- Refusal to report: [[zizhi-tongjian-hanji-870-gudai-tizhinei-buganle-meiyou-tuilu-lpwbniu9ifpt6cp4ip9czqwsh5xd|汉纪 870]] says Hu Fang ignores the matter instead of reporting it, because the court values the talent.

## Qualifications

The current page does not establish Hu Fang's full career, factional alignment, or later actions. The source gives enough evidence for his role in Huangfu Gui's failed exit, but not enough to build a broader political profile.

## What Changed

- Created a source-bounded Hu Fang page from 汉纪 870.

## Relationships

- [[HuangfuGuiLateHan|皇甫规]] - official whose attempted self-impeachment Hu Fang declines to validate.
- [[OfficeDutyExitConstraint|任官责任退出受限]] - concept grounded by Hu Fang's refusal to assist an exit tactic.
- [[BingzhouLateHan|并州]] - inspector jurisdiction attached to Hu Fang in the source.
- [[EmperorHuanOfHan|汉桓帝]] - ruler whose appointment field creates the need to keep Huangfu Gui in service.
- [[ZhangHuanLateHan|张奂]] - neighboring frontier official in the same episode's talent-dependence pattern.
