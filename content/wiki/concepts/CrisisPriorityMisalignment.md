...
tags: [governance, crisis-management, strategy, xin-dynasty]
sources:
...
危机优先级错置 / crisis priority misalignment is the governance failure in which a ruler treats the most symbolically salient or face-threatening problem as urgent while the materially closer destabilizing problem is already breaking the state's base.
...
- Crisis ranking is a state-capacity decision, not a matter of prestige alone.
- A frontier threat can be real while still being less urgent than internal collapse.
- Misalignment becomes dangerous when the chosen priority imposes new taxes, labor, and requisitions on the already unstable base.
- Accurate priority advice can be punished when it challenges the ruler's preferred display of strength.
- The pattern differs from ordinary failed strategy because the warning names a better sequence before the mistake is completed.
...
The concept does not claim the Xiongnu threat was imaginary. The source says Xiongnu raids and frontier disorder were worsening. The narrower claim is sequencing: Wang Mang's frontier response consumed domestic capacity while internal rebellion was already the nearer regime-breaking danger.
...
- [[UnsupportedFrontierIntervention|无支撑边疆干预]] - related mechanism because the Xiongnu intervention creates commitments that the domestic base cannot support.
- [[FiscalExtractionRebellionSpiral|财政压榨叛乱螺旋]] - downstream burden when the wrong priority is funded through new levies and requisitions.
- [[DirectRemonstranceCriminalization|直谏入罪]] - speech-risk neighbor because the priority warning costs Yan You office.
- [[SuppressedRootCauseReporting|根因上报压制]] - later late-Xin information failure when Wang Mang also forbids hunger-based explanations.
- [[RootCausePacification|查因安抚式治理]] - repair contrast because sequence should follow causal diagnosis.
