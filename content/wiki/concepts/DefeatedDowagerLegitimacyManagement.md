...
tags: [late-han, succession, legitimacy, ritual, court-politics]
sources:
...
Defeated dowager legitimacy management / 失势太后名分处置 is the post-succession problem of handling a defeated empress dowager or regent whose kin network has lost power but whose formal title still affects the new ruler's legitimacy.
...
- A defeated dowager can remain politically powerful through title and ritual status even after losing practical control.
- The winning succession camp may need to destroy a rival kin network while preserving the dowager's public dignity.
- Confinement is risky when it makes a later illness or death look like hidden regime violence.
- Classical filial precedents can be used as a practical legitimacy argument, not only as moral ornament.
- Visible court ritual can separate "we defeated the faction" from "we humiliated the office."
- The strategy is not the same as mercy: it can coexist with harsh purge and containment.
...
- [[SuccessionLegitimacyPurge|继位合法性清洗]] - overlapping mechanism because the same victory may require eliminating rival supporters while managing formal legitimacy.
- [[ImperialBurialLegitimacy|帝后合葬名分]] - related ritual-status problem where posthumous treatment decides whether an empress dowager remains inside official order.
- [[MingqiLegitimacy|名器合法性]] - broader title-and-symbol frame that explains why a defeated dowager's name and rites still matter.
- [[OuterRelativeCourtPower|外戚专权与清算]] - family-power field that creates the defeated-dowager problem after a kin faction collapses.
- [[AllusiveRemonstrance|隐语进谏]] - adjacent speech strategy because Zhou Ju uses classical stories to make a dangerous warning acceptable.
- [[CourtFeedbackCollapse|君臣反馈失灵]] - contrast pattern because legitimacy management depends on advisers still being able to warn the ruler before retaliation or secrecy hardens.
