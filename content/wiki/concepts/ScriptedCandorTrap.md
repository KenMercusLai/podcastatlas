...
tags: [governance, remonstrance, procedure, court-politics, wang-mang]
sources:
...
Scripted candor trap / 直言作秀陷阱 is the governance pattern where authority publicly asks for blunt speech but privately expects confirmation, praise, or ritual participation. The trap appears when a participant takes the invitation literally and the resulting correction is treated as offense.
...
- A public call for candid speech can be ceremonial if the desired answer has already been defined.
- The trap is exposed when a participant treats the invitation as genuine and supplies information that disrupts the authority holder's control.
- Punishment often depends on recoding correction as disloyalty, ideological error, procedural impropriety, or hidden motive.
- The damage is institutional because future speakers learn to supply praise or silence instead of usable diagnosis.
- The pattern can be narrated through court politics or workplace analogy, but its core is the gap between advertised deliberation and actual decision control.
...
- [[CourtFeedbackCollapse|君臣反馈失灵]] - broader institutional failure produced when useful correction cannot safely reach authority.
- [[DirectRemonstranceCriminalization|直谏入罪]] - downstream punishment pattern when explicit correction is recoded as offense.
- [[PredeterminedMeetingProcess|既定结果会议程序]] - adjacent procedure pattern where formal deliberation ratifies an already shaped decision.
- [[AdviceFramingUnderHierarchy|等级场景中的进言包装]] - contrast because some advice survives only when phrased for hierarchy, while scripted candor punishes literal frankness.
- [[PublicMoralPerformance|公开道德表演]] - related visibility pattern because the candor request helps Wang Mang advertise openness and virtue.
