...
- Identity documents, face scans, and behavioral signals can exclude some minors while burdening lawful users with error and sensitive-data exposure.
- Accuracy is especially difficult close to a legal threshold, where a one-year classification error changes access.
...
Age checks may reduce access at compliant services while displacing users to noncompliant sites or evasion methods. Device-level checks can reduce repeated disclosure yet concentrate eligibility power in dominant operating systems and app stores. Real-ID claims in the newest source are tied to Malaysia's implementation experience and should not be generalized without comparative evidence. Minimum-information assurance is a design principle, not proof that an implementation will remain narrow over time.

## What Changed
- Added direct implementation evidence about false ages, shared accounts, and relatives' identity details.
- Added the near-threshold accuracy problem for facial and behavioral estimation.
- Added real-ID document storage as both a stronger binding mechanism and a concentrated security risk.
- Strengthened the judgment that every enforcement location reallocates rather than eliminates trust and liability.
