---
title: "饥附饱扬 / Ji Fu Bao Yang"
type: concept
tags: [idiom, political-psychology, alliance, late-han]
sources: [zizhi-tongjian-hanji-1018-ta-weihe-rang-caocao-jidan-sunce-weiju-liubei-wanxi-lqccdhrjmero-gt7vsaq2ejuhhw8]
last_updated: 2026-08-23
---

# 饥附饱扬 / Ji Fu Bao Yang

饥附饱扬 / Ji Fu Bao Yang enters the wiki through [[zizhi-tongjian-hanji-1018-ta-weihe-rang-caocao-jidan-sunce-weiju-liubei-wanxi-lqccdhrjmero-gt7vsaq2ejuhhw8|汉纪 1018]] as [[ChenDeng|陈登]]'s reported description of how [[CaoCao|曹操]] judges [[LyuBu|吕布]]. The phrase names a dependency pattern: when hungry or constrained, a person attaches; once fed or secure, that person lifts off and leaves.

The concept is not a general claim that all alliance behavior is selfish. Its wiki use is narrower: it marks a leader whose cooperation may depend on continued need rather than trust, institutional commitment, or shared durable interest. In the source, this lets Chen Deng turn Cao Cao's suspicion into a face-saving explanation Lü Bu can accept.

## Key Claims
- Need can create temporary obedience without creating durable loyalty.
- Feeding, rewarding, or empowering a volatile actor can reduce dependency and increase exit risk.
- A mediator can make a hostile judgment usable by translating it into the target's vanity and self-protection language.
- The pattern belongs near [[InterestStructuredAllianceFragility|利益结构式联盟脆弱性]] because both ask whether apparent cooperation rests on durable interests.

## Connections
- [[ChenDeng|陈登]] - speaker who reports and reframes the judgment.
- [[CaoCao|曹操]] - actor whose caution supplies the "raise a tiger" image.
- [[LyuBu|吕布]] - target of the dependency diagnosis.
- [[InterestStructuredAllianceFragility|利益结构式联盟脆弱性]], [[InterestReframingPersuasion|利益重构式说服]], and [[GraceDebtLoyaltyPressure|恩赏负债式忠诚压力]] - adjacent concepts about incentives, attachment, and loyalty pressure.
