...
tags: [governance, local-administration, famine-relief, western-han]
sources:
...
民生优先安抚治理 / livelihood-first pacification is the governance pattern where officials treat disorder rooted in hunger, scarcity, and official neglect as a repairable livelihood problem before classifying the population as enemies to suppress.
...
- Not all disorder should be read first as criminality; hunger and official neglect can be the prior cause.
- Effective pacification starts by asking whether the state intends to defeat subjects or restore them.
- Clear behavioral classification can reduce indiscriminate repression by separating farmers with tools from armed robbers.
- Relief must be paired with production recovery, thrift discipline, and official presence to become durable order.
- The method depends on superior authorization because the official must be protected from rushed punitive interference.
...
This concept is currently grounded in one exemplary Western Han source note. It should not be generalized into a claim that coercion is never needed, since Gong Sui still treats armed people as robbers and needs imperial backing. The current claim is narrower: when unrest is driven by hunger and official neglect, livelihood repair can be the main security instrument.
...
- [[GongSuiWesternHan]] - official whose Bohai method anchors the concept.
- [[HumaneLocalGovernance]] - broader local-governance family that includes relief, restraint, diagnosis, and proportionate correction.
- [[PopularSufferingGovernanceDiagnostic]] - upstream diagnosis that converts misery and disorder into evidence about official responsibility.
- [[RootCausePacification]] - adjacent concept for solving disorder by repairing causes rather than only attacking symptoms.
- [[RiskBearingReliefGovernance]] - related relief pattern because opening granaries can require official responsibility and superior tolerance.
- [[AppointmentQualityAsStateCapacity]] - enabling condition because Xuan's appointment of Gong Sui gives the right official a matching problem.
