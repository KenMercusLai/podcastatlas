...
type: concept
tags: [frontier, governance, late-han, western-regions, information]
sources:
...
Rumor-driven frontier intervention / 谣言驱动的边疆干预 is the failure mode where a frontier official treats an unverified accusation from a local rival as actionable truth, then intervenes coercively in a dependent polity and produces wider instability.
...
- Frontier information is often interested information: local rivals can convert private enmity into a story aimed at Han authority.
- Investigation authority becomes dangerous when a field official treats accusation as proof before checking incentives and evidence.
- Coercive surprise, such as a banquet seizure, can destroy the legitimacy of central authority even when the official claims imperial command.
- A failed intervention can create multiple instability layers: anti-Han retaliation, self-enthronement, popular rejection, and emergency succession repair.
- The pattern differs from ordinary frontier rebellion because the trigger is the center's representative acting on captured information.
...
This concept is grounded in one source note and should not be generalized to every Western Regions conflict. The source does not prove whether Zhao Ping was murdered; the point is that Wang Jing acts before the accusation has become reliable. Ma Da's punitive impulse and Song Liang's later manipulation attempt remain source-scoped because the current concept centers on Wang Jing's initial conversion of rumor into coercive intervention.
...
- [[RecognitionBackedFrontierDiplomacy|册封威慑式边疆外交]] - contrast because recognition contains frontier risk through status settlement, while Wang Jing's intervention destroys credibility.
- [[UnauthorizedFrontierCommandIntervention|边疆将领擅断干预]] - adjacent pattern where field authority changes a dependent polity's leadership without stable central authorization.
- [[FrontierCommandCollapse|边郡军政崩盘]] - related failure mode where frontier governance breaks before ordinary military success is possible.
- [[OverextendedFrontierExpedition|边境远征过度]] - contrast because the Khotan case is not excessive expedition scale but excessive trust in captured information.
- [[CourtFeedbackCollapse|君臣反馈失灵]] - broader governance failure when bad information cannot be corrected before action.
