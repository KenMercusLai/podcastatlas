...
[[e249-token-jingji-zhuandian-openclaw-hermes-dao-bendi-ziyan-de-agent-jinhua-zhi-lu-6242033d-a14a-44e3-a622-cbfc7d3c3817]] adds a sharper boundary from [[Dongxu]]: memory remains an open problem even after products such as [[OpenClaw]] and [[HermesAgent]]. The episode says repeated preference loss, rollback behavior, and memory confusion can waste tokens, while Hermes' practical improvement is to turn successful experience into [[AISkills]] rather than solve all memory quality issues.
...
[[yong-agent-donglixue-he-40-ge-agents-yiqi-wei-ren-ai-zuo-chanpin-duitan-slock-ai-chuangshiren-rc-liiv-fkcdolfb06hkoyz0ix3fejy]] adds a shared-memory version through [[SlockAI|Slock.ai]]. [[RC]] describes memory as something agents can observe together, learn from across sessions, and potentially fork when one person's agent has accumulated valuable experience, which links memory to [[AgentDynamics]] and [[AgentMarketplace]] rather than only personalization.

[[women-shi-ruhe-dingyi-openclaw-for-teams-xin-chanpin-xingtai-de-duitan-kuse-junior-lianchuang-jian-cto-yuhao-lkp1a0todflxoyycyo3zhrap3ebv]] adds the [[EnterpriseAgentMemory]] boundary through [[Junior]]. [[Yuhao]] argues that company agents cannot simply reuse personal-assistant memory: they should remember project state, company priorities, organizational relationships, and privacy boundaries from the company's perspective.

[[ai-bu-zhi-bi-zhishang-waic-he-kimi-k3-toulule-shenme-xin-jingzheng-1]] adds a project-knowledge-file practice. The host describes having a model summarize the day's work at the end of a session and write it into a knowledge folder so later sessions can recover prior decisions, context, and failure notes. This is a lightweight memory layer rather than a fully automatic product memory system, but it reinforces that durable AI work needs explicit context preservation when chat context is deleted or expires.

[[tsr-s3-dansiroker-v3-tsr-s3-dansiroker-v3]] adds [[RewindAI]] and [[Limitless]] as an earlier personal-memory product case. [[DanSiroker]] frames memory around what a user has seen, said, and heard; Rewind captures local screen and audio context, while Limitless moves toward cloud-based AI and wearable conversation capture. This source sharpens the privacy boundary: [[PersonalAIMemory]] is useful because it is intimate, and risky for the same reason.
...
- In multi-agent workspaces, memory can become group infrastructure: corrections, task history, and reusable lessons can transfer across agents instead of staying inside one chat.
- Personal memory products need capture and consent design because the richest user context may include bystanders, meetings, screens, and private conversations.
- Enterprise memory changes the memory owner from one person to an organization, so retrieval must consider role, project, permission, and sensitive-disclosure boundaries.
- Memory salience is part of memory quality: a saved fact can be correct but still wrong to mention because the context, sensitivity, or conversational frame does not fit.
- A simple knowledge-folder habit can be useful before a full memory product exists, especially for AI coding sessions where prior fixes and project decisions easily fall out of context.
- E249 adds that bad memory is also a cost problem: repeated re-teaching, rollback, and context confusion consume tokens without improving the accepted result.

## Connections
- [[ContextEngineering]] — broader discipline for making context durable and useful.
- [[JanelleShane]], [[Claude]], [[PersonalAIMemory]], and [[ChatbotMemorySalienceFailure]] — consumer-chatbot memory salience case added by Marketplace Tech.
...
- [[SlockAI|Slock.ai]], [[AgentDynamics]], [[AgentMarketplace]], and [[AgentTaskClaiming]] — shared multi-agent memory branch added by the RC episode.
- [[RewindAI]], [[Limitless]], [[DanSiroker]], [[PersonalAIMemory]], and [[ConsentBasedRecording]] — personal AI memory and wearable-capture branch added by The Social Radars.
- [[Kuse]], [[Junior]], [[OpenClawForTeams]], and [[EnterpriseAgentMemory]] — company-centered memory branch added by the Yuhao source.
- [[KimiK3]], [[AIEngineeringThinking]], and [[AICodingVerification]] — project-knowledge-file practice added by the Kimi K3 episode.
- [[Dongxu]], [[OpenClaw]], [[HermesAgent]], [[AISkills]], and [[TokenEfficientAgentWorkflow]] — E249's memory-open-question and skill-substitution boundary.
