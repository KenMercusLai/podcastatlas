---
title: "帝王死因史料推断 / Imperial Death-Cause Inference"
type: concept
knowledge_schema: synthesis-v1
tags: [historiography, health, imperial-death, evidence]
sources:
  - zizhi-tongjian-hanji-732-donghan-guangwudi-liuxiu-jiabeng-bingsi-de-lnsu1le2x-cj75cjslysvbndeuvr
last_updated: 2026-08-31
---

## Definition

帝王死因史料推断 / Imperial Death-Cause Inference is the practice of reading sparse death notices together with earlier symptom records, workload evidence, and source comparison while keeping diagnosis bounded by what the sources actually say.

## Current Synthesis

[[zizhi-tongjian-hanji-732-donghan-guangwudi-liuxiu-jiabeng-bingsi-de-lnsu1le2x-cj75cjslysvbndeuvr|汉纪 732]] grounds the concept through [[LiuXiu|光武帝刘秀]]. [[ZizhiTongjian|《资治通鉴》]] records his death in terse court terms: place, age, and date. The episode then uses [[DongguanHanji|《东观汉记》]] notices of earlier dizziness, immobility, cold/wind illness, and jaundice-like symptoms to ask whether Liu Xiu's death followed a longer health decline.

The synthesis is methodological rather than diagnostic. The useful judgment is that a founder's late health can be historically legible without a named disease. Symptom clues, chronic overwork, and reading/state-business habits all sharpen the question, but the evidence does not justify converting the question into a settled medical answer.

## Key Claims

- Terse imperial death notices often identify the event but leave cause, course, and clinical context underdetermined.
- Supplementary source traditions can add symptom history, but symptom history is not the same thing as diagnosis.
- Ruler workload and self-description can explain plausible health pressure while still remaining indirect evidence.
- Responsible inference separates "this pattern is suggestive" from "this disease caused the death."
- In Liu Xiu's case, the strongest current claim is long-term health vulnerability, not a definitive cause of death.

## Evidence

- Sparse death notice: [[zizhi-tongjian-hanji-732-donghan-guangwudi-liuxiu-jiabeng-bingsi-de-lnsu1le2x-cj75cjslysvbndeuvr|汉纪 732]] says Zizhi Tongjian gives Liu Xiu's death at Nangong in a short date/place/age form.
- Supplementary symptoms: [[zizhi-tongjian-hanji-732-donghan-guangwudi-liuxiu-jiabeng-bingsi-de-lnsu1le2x-cj75cjslysvbndeuvr|汉纪 732]] uses Dongguan Hanji notices from 41 CE and 44 CE to add earlier illness clues.
- Workload pressure: [[zizhi-tongjian-hanji-732-donghan-guangwudi-liuxiu-jiabeng-bingsi-de-lnsu1le2x-cj75cjslysvbndeuvr|汉纪 732]] describes Liu Xiu's long court days, late-night work, and refusal to treat governing as tiring.
- Inference boundary: [[zizhi-tongjian-hanji-732-donghan-guangwudi-liuxiu-jiabeng-bingsi-de-lnsu1le2x-cj75cjslysvbndeuvr|汉纪 732]] keeps the death-cause discussion indirect rather than naming a settled disease.

## Counterevidence & Qualifications

The source does not give an autopsy, a final illness narrative, or a direct causal statement connecting earlier symptoms to death in 57 CE. Modern disease names should not be retrofitted onto the episode without stronger source evidence. The concept also differs from cases where a source directly reports concealed death, poison, murder, or a clear acute event.

## What Changed

- Created this concept from 汉纪 732 to capture the source's bounded method for reading Liu Xiu's death.

## Related Concepts

- [[ChronicleSourceGaps|史书缺环]] - supplies the broader problem of silence or compression in transmitted narrative.
- [[ObservationBeforeInference|先观察后推断]] - provides the methodological discipline of separating observed clues from conclusions.
- [[ForensicDeathTimeInference|死亡时间法医推断]] - adjacent inference pattern, but focused on death timing rather than imperial cause of death.
- [[FounderHealthDebt|创始人健康债]] - related health-cost frame for high-responsibility founding figures.
- [[ImperialDeathConcealment|帝王之死隐匿]] - contrast case where the political problem is hiding death, not inferring cause from sparse evidence.
