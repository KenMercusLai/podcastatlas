---
title: "经学考试正统化 / Classical Examination Orthodoxy"
type: concept
knowledge_schema: synthesis-v1
tags: [education, examinations, classics, eastern-han, taixue]
sources:
  - zizhi-tongjian-hanji-776-hanshi-tanmi-cailun-shi-zenyang-faming-zaozhishu-de-ljqhhcywxr-yggzmcgzbbtg-q-hb
last_updated: 2026-08-30
---

## Definition

经学考试正统化 / Classical Examination Orthodoxy is the institutional pattern where classical-learning examinations are constrained to recognized textual lineages, explicit passages, and approved interpretive points in order to reduce free-form argument and arbitrary grading.

## Current Synthesis

汉纪 776 grounds the concept in [[XuFangLateHan|Xu Fang]]'s 102 CE memorial about [[Taixue|Taixue]]. He complains that doctoral disciples are being examined in ways that let personal opinion replace the teachings of their schools, while exam participants cover for one another and dispute meanings without firm textual basis. His proposal is to build examinations around fixed points from classical writings and to reward answers with clear source grounding.

The source presents standardization as institutional repair: exams become easier to judge when they can be checked against a recognized textual framework. The same move also narrows interpretive freedom, so the concept should be read as a stability-and-control mechanism rather than as pure intellectual improvement.

## Key Claims

- Classical examinations can fail when students and examiners replace textual lineage with free-form personal opinion.
- Standardized points and explicit source references make assessment more auditable.
- Orthodoxy can repair grading conflict while also narrowing permissible interpretation.
- Education governance here is tied to state capacity because official learning depends on credible testing.

## Evidence

- Diagnosis: [[zizhi-tongjian-hanji-776-hanshi-tanmi-cailun-shi-zenyang-faming-zaozhishu-de-ljqhhcywxr-yggzmcgzbbtg-q-hb|汉纪 776]] says Xu Fang criticizes Taixue examination practices for relying on personal opinion and mutual tolerance rather than the students' inherited classical traditions.
- Reform design: [[zizhi-tongjian-hanji-776-hanshi-tanmi-cailun-shi-zenyang-faming-zaozhishu-de-ljqhhcywxr-yggzmcgzbbtg-q-hb|汉纪 776]] says Xu Fang proposes using fifty textual points by discipline, treating detailed explanation and clear citation as superior.
- Enforcement boundary: [[zizhi-tongjian-hanji-776-hanshi-tanmi-cailun-shi-zenyang-faming-zaozhishu-de-ljqhhcywxr-yggzmcgzbbtg-q-hb|汉纪 776]] says answers departing from teacher-lineage doctrine and relying on personal conflict are to be marked wrong.

## Counterevidence & Qualifications

The source does not show whether Xu Fang's standards improved learning outcomes, how long they lasted, or how much disagreement they suppressed. The concept therefore captures a bounded examination-repair logic: textual orthodoxy improves assessment legibility while potentially reducing interpretive openness.

## What Changed

- Created the concept from 汉纪 776's account of Xu Fang's Taixue examination reform.

## Related Concepts

- [[Taixue|太学]] - institutional site where the examination reform is applied.
- [[AppointmentQualityAsStateCapacity|任官适任性国家能力]] - related governance frame because official quality depends on credible education and testing channels.
- [[OfficialLearningMonopoly|学在王官式知识垄断]] - broader pattern of state-linked control over recognized learning.
- [[OfficialSelectionChannelCompetition|选官通道竞争]] - later selection-channel conflict that depends on which curriculum and credentials count for office.
- [[XipingStoneClassics|熹平石经]] - later public text-standardization project that also makes classical authority more checkable.
