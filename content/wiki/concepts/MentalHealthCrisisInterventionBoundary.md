---
title: "Mental Health Crisis Intervention Boundary / 心理危机干预边界"
type: concept
tags: [mental-health, crisis, therapy, safety]
sources: [ep236-lao-wu-kuai-qian-xinlihua-fufei-de-peiban-shi-zhende-ma-ltvuoeluoeu4-a3j3lamhc3buudx]
last_updated: 2026-08-07
---

# Mental Health Crisis Intervention Boundary / 心理危机干预边界

Mental health crisis intervention boundary / 心理危机干预边界 is the limit at which ordinary support, paid listening, or AI companionship is no longer enough and trained escalation is needed. In [[ep236-lao-wu-kuai-qian-xinlihua-fufei-de-peiban-shi-zhende-ma-ltvuoeluoeu4-a3j3lamhc3buudx]], listeners may encounter users who send self-harm videos or make life-threatening statements, while ordinary shops can only comfort, judge credibility, and sometimes contact local police.

The concept extends [[TherapyRelationshipAndBoundaries]] by making the risk operational. The issue is not only whether a helper feels kind; crisis work needs recognition, reporting, emergency contacts, role limits, and the ability to move from conversation to protection when danger is credible.

## Key Claims
- Self-harm, suicide threats, severe functional collapse, and life-threatening behavior exceed ordinary paid-listening capacity.
- Crisis boundaries should be decided before the incident, not improvised after a listener is emotionally overwhelmed.
- The helper's empathy can become unsafe if it delays escalation, hides danger, or accepts secrecy as intimacy.
- Professional therapy and counseling systems matter partly because they create procedures for risk assessment and referral.
- AI companions face a parallel version of the same boundary when long sessions validate crisis framing without escalation.
- Friends and family also need boundaries: witnessing matters, but severe danger should lead toward emergency or professional support.

## Connections
- [[ListeningTherapyBoundary]], [[PaidListeningServices]], and [[EmotionalServicePlatformIncentives]] - service context where the boundary appears.
- [[TherapyRelationshipAndBoundaries]], [[GriefCompanionSupport]], and [[TraumaNarrativeIntegration]] - professional and informal support branch.
- [[TeenChatbotMentalHealthRisk]], [[AIPsychosis]], [[SycophanticAICompanionRisk]], and [[ChatbotSafetyGuardrailDecay]] - AI mental-health safety parallels.
- [[EmpathyBoundaries]] and [[CommunicationBoundarySetting]] - relationship practices that keep care from becoming dangerous over-identification.
