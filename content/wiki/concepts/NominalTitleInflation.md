...
tags: [institutions, rank, governance, xin-dynasty, status]
sources:
...
名义爵位通胀 / nominal title inflation is the institutional failure in which a ruler proliferates ranks, fiefs, or noble titles faster than the state can attach land, revenue, duty, and real administrative support to them.
...
- Rank systems need material support; title creation without land or revenue converts honor into dependency.
- Proliferation lowers status value because scarcity and achievement signals weaken.
- Retro-classical design can produce institutional fiction when implementation capacity is missing.
- Title inflation harms both elites and state credibility because recognized people must still bargain for subsistence.
- The failure differs from ordinary poverty because the state has publicly promised rank while withholding the practical base that makes rank meaningful.
- Title redesign can also be subtractive: downgrading old-dynasty titles and replacing them with new grades changes political status even before material support is addressed.
...
- [[MilitaryMeritRankSystem|军功爵制]] - contrast because merit-rank scarcity anchors older title value in service and reward.
- [[TitleRealityRestraint|名实克制]] - related norm that titles should not outrun reality.
- [[WangMangReforms|王莽改制]] - broader reform field that generates the source case.
- [[MingqiLegitimacy|名器合法性]] - wider title-and-symbol frame explaining why downgrading kings and marquises matters.
- [[AdministrativeRenamingOverreach|行政改名过度]] - neighboring Xin failure where symbolic redesign creates operating confusion.
- [[InstitutionalOvercompliance|制度性过度服从]] - adjacent institutional pathology where formal categories displace practical function.
