...
tags: [conflict, workplace, governance, communication, court-politics]
sources:
...
Sincere power-sharing de-escalation is a conflict pattern where rivals with overlapping authority reduce factional competition by openly consulting, sharing decisions, and grounding cooperation in a common institutional interest rather than in hidden balancing or tactical politeness.
...
The modern workplace translation is narrow but useful: some apparent "deadlocks" are not solved by more sophisticated factional play. When the competing parties can still accept shared goals, visible and sincere joint process can turn overlapping authority from a zero-sum personnel fight into coordinated governance.

## Key Claims

- Overlapping personnel or recommendation authority can turn colleagues, relatives, or partners into rivals even without open hostility.
- A third party can lower conflict by reframing rivalry around shared identity, reputation, and organizational interest.
- Sincerity matters because the repair depends on actual consultation and decision sharing, not only polite speech.
- This pattern can defeat a superior's divide-and-balance design when the would-be rivals maintain a cooperative front.
- The method depends on some remaining capacity for trust; it is weaker when material interests are non-negotiable or one side is predatory.
...
- [[RespectBasedConflictDeescalation|尊重式冲突降温]] - adjacent de-escalation pattern based on credible recognition rather than shared decision process.
- [[WorkplaceRelationshipBoundaries]] - workplace frame for separating colleague, kinship, faction, and authority roles.
- [[OuterRelativeCourtPower|外戚专权与清算]] - court-power field where Wang-family internal unity matters.
- [[StrategicForbearanceGovernance]] - broader restraint pattern that can preserve cooperation instead of escalating immediately.
- [[StructuralPowerImbalance]] - diagnostic complement because sincere consultation is risky when authority and retaliation are asymmetrical.
