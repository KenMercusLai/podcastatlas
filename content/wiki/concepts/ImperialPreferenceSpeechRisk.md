...
tags: [court-politics, remonstrance, communication, eastern-han]
sources:
...
- Advice risk rises when the ruler's preferred framework also supports personal legitimacy.
- Direct evidence-based correction can be recoded as offense even when it is intellectually coherent.
- Ambiguity can reduce immediate danger by giving the ruler a face-saving exit.
- Accommodation can be rewarded when it makes scholarship serve the ruler's desired frame.
- The pattern helps explain why court knowledge can shift from inquiry toward confirmation.
...
This is a single-source concept and should not be generalized to every ruler-subordinate conversation. The source morally favors Huan Tan's intellectual honesty, but it also shows that survival strategies can be rational under severe asymmetry. The concept should be used where ruler preference changes the accepted evidentiary frame, not merely where an official phrases advice tactfully.
...
- [[AdviceFramingUnderHierarchy|等级场景中的进言包装]] - broader communication pattern that includes but is not limited to legitimacy-sensitive advice.
- [[ChenweiEvidentiaryCritique|谶纬证据批判]] - evidence standard that becomes risky when it opposes the ruler's framework.
- [[ChenweiPolitics|谶纬政治]] - ideological system whose court use grounds the current case.
- [[DirectRemonstranceCriminalization|直谏入罪]] - danger pattern where correction is treated as punishable offense.
- [[CourtFeedbackCollapse|君臣反馈失灵]] - downstream risk when officials learn to confirm rather than correct.
