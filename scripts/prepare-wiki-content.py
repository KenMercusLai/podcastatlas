...
def alphabetical_bucket(key: str) -> str:
    """Return the static 0-9/A-Z bucket for a canonical wiki key."""
    first = key[:1]
    if first.isascii() and first.isdigit():
        return "0-9"
    if first.isascii() and first.isalpha():
        return first.lower()
    raise ValueError(f"Unsupported canonical wiki key: {key!r}")
...
def expected_alphabetical_files(pages: list[WikiPage]) -> dict[Path, str]:
    ...
    for section in ALPHABETICAL_SECTIONS:
        ...
        for bucket in ALPHABETICAL_BUCKETS:
            generated[section_dir / "by-letter" / bucket / "_index.md"] = alphabetical_index(
                section,
                bucket,
                groups[bucket],
                total_pages,
...
def skip_link_scan(path: Path) -> bool:
    return path.name == "_index.md" or path == STATS_PATH
